...
export default function EditMonitor() {
    ...
    async function requisicaoCurrentUser(){
        ...
        setLoading(true);
        const dadosUser = await userService.getMonitorById(id);
        if(dadosUser){
    ...
    async function handleSave(e){
        ...
        const response = await userService.saveMonitorById(data,id);
        setLoading(false);
        setLoading(true);
    ...
    if(loading){
        ...
        return (
            <div>
                <BreadCrumbs rotas={[{"rota":"Painel","link":"/monitor"},{"rota":"Monitor","link":"/monitor/monitor"},{"rota":`Editar Monitor: ${nome}`,"link":`/monitor/monitor/editar-monitor/${id}`}]} />
